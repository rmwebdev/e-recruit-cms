        ...
        init: function (options) {
            ...
            if (target.length > 0) {
                ...
                if (!settings.hideRowNumColumn) {
                    ...
                    tbHeadCellRowNum.innerHTML = decode64("Tm8=");
                ...
                if (!settings._hideLastColumn) {
                    ...
                    tbHeadCellRowButton.innerHTML = decode64("QWN0aW9ucw==");
                    // Add column group for scrolling
                    tbColGp.appendChild(document.createElement('col'));
                }
    ...
    function insertRow(tbWhole, numOfRowOrRowArray, rowIndex, callerUniqueIndex) {
        ...
        for (var z = 0; z < numOfRow; z++) {
            ...
            for (var y = 0; y < settings.columns.length; y++) {
                ...
                if (settings.columns[y].type == 'custom') {
                ...
                else if(settings.columns[y].type == 'inputwithclear'){
                    ctrl = document.createElement('input');
                    ctrl.type = 'text';
                    ctrl.id = ctrlId;
                    ctrl.name = ctrlName;
                    ...
                    btn.type = 'button';
                    btn.id = settings.idPrefix + '_' + settings.columns[y].name + '_' + 'remove' + '_' + uniqueIndex, ctrlName;
                    ctrlBeforeColumn = settings.idPrefix + '_' + settings.columns[y-1].name + '_' + uniqueIndex, ctrlName;
                    btn.setAttribute('onClick', "document.getElementById('"+ctrlId+"').value='';document.getElementById('"+ctrlBeforeColumn+"').value='';");
                    btn.setAttribute('class', "btn-danger");
                    var t = document.createTextNode("X");
                    btn.appendChild(t);
                    spn.appendChild(btn);
                    tbCell.appendChild(spn);
  
                }
                else if(settings.columns[y].type == 'select2'){
                    ctrl = document.createElement('select');
                    ctrl.id = ctrlId;
                    ctrl.name = ctrlName;
                    ctrl.className = "select2append";
                    ...
                    } else if (settings.columns[y].type == 'ui-datetimepicker') {
                        $(ctrl).datetimepicker(settings.columns[y].uiOption);
                    ...
                    } else if (settings.columns[y].type == 'ui-timepicker') {
                        $(ctrl).clockpicker(settings.columns[y].uiOption);
                    }
                }
    ...
    function getCtrlValue(settings, colIndex, uniqueIndex) {
        ...
        if (type == 'checkbox') {
            ...
            if (ctrl == null)
                ...
                if(type == 'select2')
                    return $('#'+settings.idPrefix + '_' + columnName + '_' + uniqueIndex).val();
                else
                    return ctrl.value;
        }
    }
